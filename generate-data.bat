@echo off
echo [THOR] Generating data.js from project 02-Database/thor-pricing.json5...
powershell -Command "// THOR 索而橱柜报价系统 - 产品数据库
// 源数据来自: THOR索而家居厨柜类价格表-2025.xlsx + 五金.xlsx
// 最后更新: 2026-06-17
...
} = Get-Content '02-Database\thor-pricing.json5' -Raw; // THOR 报价数据库 - 由 02-Database/thor-pricing.json5 生成
...
} -replace '//.*',''; // THOR 报价数据库 - 由 02-Database/thor-pricing.json5 生成
// 更新价格时: edit json5 → run generate script → git push
const THOR_DATA = 
...
} = '// THOR 报价数据库 - 由 02-Database/thor-pricing.json5 生成' + [char]13 + [char]10 + '// 更新价格时: edit json5 run this script' + [char]13 + [char]10 + 'const THOR_DATA = ' + // THOR 报价数据库 - 由 02-Database/thor-pricing.json5 生成
// 更新价格时: edit json5 → run generate script → git push
const THOR_DATA = 
...
}; Set-Content 'data.js' // THOR 报价数据库 - 由 02-Database/thor-pricing.json5 生成
// 更新价格时: edit json5 → run generate script → git push
const THOR_DATA = 
...
echo Done! Run: git add . && git commit -m "update prices" && git push
pause